...
class Chessboard(classroom.Classroom):
    ...
    def is_fight(self,q1,q2):
        return q1.x==q2.x or q1.y==q2.y or abs(q1.x-q2.x)==abs(q1.y-q2.y)
    def how_fight(self,q,qq):
        ...
        return lis
    def move(self,q):
        futs=[Queen(x,y) for x in [(q.x)+1,q.x,(q.x)-1] for y in [(q.y)+1,q.y,(q.y)-1] if x<self.n if y<self.n if x>=0 if y>=0 if self.a[x][y]!=1]
        minf=q
        for qu in futs:
